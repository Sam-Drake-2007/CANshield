...
import getBoats from "../../services/getBoats.js";
...
// Optional: used in sidebar "Previous Operations" (still mock for now)
...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setBoats([]);
        setBoatsError(e?.message || "Failed to load boats");
      } finally {
        if (!alive) return;
  ...
  const boatsList = boats.length ? boats : BOATS_FALLBACK;
  ...
  // init qty map once boats are known
  useEffect(() => {
    if (Object.keys(qtyByBoatId).length) return;
    setQtyByBoatId(Object.fromEntries(boatsList.map((b) => [b.id, 0])));
  ...
  const totalShips = useMemo(() => {
    return boatsList.reduce((acc, b) => acc + (qtyByBoatId[b.id] || 0), 0);
  }, [boatsList, qtyByBoatId]);

  const totalCost = useMemo(() => {
    return boatsList.reduce((acc, b) => acc + (qtyByBoatId[b.id] || 0) * (b.cost || 0), 0);
  }, [boatsList, qtyByBoatId]);
  ...
  }, [canGoToDraw, initShipsFromFleet]);
  ...
  const boatById = useMemo(
    () => Object.fromEntries(boatsList.map((b) => [b.id, b])),
    [boatsList]
  );
  ...
  const finalStats = useMemo(() => {
    const totalRoutePoints = ships.reduce((a, s) => a + (s.route?.length ?? 0), 0);
    return {
      totalCost,
      totalShips,
  ...
  }, [userId, totalCost, totalShips, ships, coverageSummary, coverageSnapshot, qtyByBoatId, boatsList]);
  ...
  const handleTimerEnd = useCallback(async () => {
    ...
    const run = {
      ...
      boats: snap.boatsList.map((b) => ({ id: b.id, name: b.name })),
    };

    try {
        await createOperation({
            userId: snap.userId ?? userId, // optional if you add userId into latestRef
            fleet: {
            ships: Object.entries(snap.qtyByBoatId)
                .filter(([_, qty]) => qty > 0)
                .map(([boatId, quantity]) => ({ boatId, quantity })),
            },
            coverage: {
            avgPercent: snap.coverageSummary.avg,
            totalPercent: snap.coverageSummary.total,
            minPercent: snap.coverageSummary.min,
            maxPercent: snap.coverageSummary.max,
            },
            totalCost: snap.totalCost,
            createdAt: snap.createdAt ?? new Date().toISOString(), // optional
        });
    } catch (e) {
  ...
  return (
    <div className="w-screen h-screen relative">
      {boatsLoading && (
        <div className="absolute bottom-2 left-2 z-[2000] text-white/70 text-xs">
          Loading boats…
        </div>
      )}
      {boatsError && (
        <div className="absolute bottom-2 left-2 z-[2000] text-red-300 text-xs">
          {boatsError}
        </div>
      )}
        ...
        {ships.map((s) =>
          (s.route?.length ?? 0) > 1 ? (
            <Polyline
              ...
              positions={s.route}
              pathOptions={{
                ...(isDraw && s.id === activeShipId ? DRAW_STYLE : DASHED_ROUTE_STYLE),
          ...
          <div className="w-[560px] max-w-[92vw] rounded-2xl border border-white/10 bg-black/60 backdrop-blur-xl shadow-[0_12px_40px_rgba(0,0,0,0.45)] p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                ...
                <div className="text-lg font-semibold text-white">{finalStats.shipsDeployed}</div>
              </div>

              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <div className="text-xs text-white/60">Duration</div>
                <div className="text-lg font-semibold text-white">{finalStats.durationSeconds}s</div>
              </div>

              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <div className="text-xs text-white/60">Total route points</div>
                <div className="text-lg font-semibold text-white">{finalStats.totalRoutePoints}</div>
              </div>
